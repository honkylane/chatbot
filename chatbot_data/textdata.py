...
class TextData:
    ...
    def extractConversationold(self, conversation):
    ...
    def extractConversation(self, conversation):
        ...
        for i in range(len(conversation["lines"])):  # We ignore the last line (no answer for it)
            try:
              ...
              if inputWords and inputLine:  # Filter wrong samples (if one of the list is empty)
                  eposide.append([ inputWords, inputLine["text"] ]) 
            except:
               print "failed "
        if eposide:
          self.trainingSamples.append([eposide])
